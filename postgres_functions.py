...
def get_dbauth():
    ...
    #                   fconfig=f'{cpath}/review/.env/dbconfig.json',
    #                   envvar='RDS_CONFIG',)
    print(cpath)
    dbauth = get_auth(ssmkey="tenx/db/strapi",
                 fconfig=f'{cpath}/review/.env/postdbconfig.json',
                 envvar='rds_CONFIG',
                 )
...
def insert_data(dbname='strapidev'):
    # try:
        db = db_connect()
        ...
        for i, row in appliInfo.iterrows():  
            ...
            sqlQuery = ''' INSERT INTO applicant_informations(time_stamp, email, firstname, english_level, commitment, self_funding,
                            ...
                            self_taught, Accept_terms_and_conditions, occupation, highest_completed_level_of_Education, graduation_date, 
                            linked_in_profile, reason_to_join, 
...
def retrive_reviewer(dbName):
    _ = db_connect(dbName)
    cur = _.cursor ()
    cur.execute(f"SELECT * from reviewers")
    res = cur.fetchall()
    return res
...
def getReviewers(reviewerGroup, dbName):
    _ = db_connect(dbName)
    cur = _.cursor ()
    cur.execute(f"SELECT * from reviewers where reviewer_group = {reviewerGroup}")
    res = cur.fetchall()
...
def fromTenx():
    ...
    # where test_score >= 20"
    df = db_execute_fetch(query, rdf=True, dbName='strapidev')
...
def update_appli_with_reviewer(dbName='strapidev'):
    ...
    for i, row in appliInfo.iterrows():
        ...
        # print(id, secondReviewerId)
        # print(id, thirdReviewerId)
        
        sqlQuery= f""" UPDATE applicant_informations SET second_reviewer_id = {secondReviewerId} WHERE id = {id}
                    """ 
...
if __name__ == "__main__":
    ...
    # query = "describe applicant_informations"
    
    batch ="batch-5"
    # print(db_connect(dbName="strapidev" ))
    # print(db_connect(dbName="strapidev" ))
    # print(cpath)
    # reviewerId = 4 applicant_informationss where batch= \'batch-5\'
    reviewerId = 4
    # query= " d+ applicant_informations"
    # and second_reviewer_id ={reviewerId}
    query = f"SELECT * from applicant_informations where batch= \'batch-5\'"
    # insert_data(dbname='strapidev')
